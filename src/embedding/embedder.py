...
def validate_chunk_records(chunk_records: list[dict]) -> list[dict]:
    ...
    for chunk_record in chunk_records:
        ...
        if not chunk_text or not chunk_text.strip():
            log.warning(
                f"Skipping chunk '{chunk_record.get('chunk_id', 'unknown')}': "
...
def generate_embeddings(model_name: str, texts: list[str], batch_size: int = 64):
    ...
    for start_index in range(0, len(texts), batch_size):
        ...
        log.info(f"Embedded batch {start_index // batch_size + 1}: {len(batch_texts)} chunk(s)")

    return all_embeddings


def attach_embeddings_to_chunks(
    chunk_records: list[dict], embeddings: list[list[float]], model_name: str
) -> list[dict]:
...
def embed_chunks(chunk_records: list[dict], model_name: str, batch_size: int = 64) -> list[dict]:
    ...
    embedded_chunk_records = attach_embeddings_to_chunks(
        valid_chunk_records, embeddings, model_name
    )
