...
def validate_embedded_chunk_records(embedded_chunk_records):
    ...
    for record in embedded_chunk_records:
        ...
        if embedding is None:
            log.warning("Embedding is None")
            continue
        elif len(embedding) == 0:
            log.warning("Embedding has length 0(empty)")
            continue
        for k, v in metadata.items():
            if v is None:
                metadata[k] = ""
                log.info(
                    f"Metadata {k} value found as None. Converted to empty string for insertion. Chunk_id : {chunk_id}"
                )
...
def prepare_records_for_insertion(embedded_chunk_records):
    ...
    log.info("Lists: ids, documents, metadatas, and embeddings - have equal length")
...
def add_records_to_collection(collection, embedded_chunk_records):
    ...
    if not valid_records:
        log.warning("Valid Records does not exist or is empty.")
        return
