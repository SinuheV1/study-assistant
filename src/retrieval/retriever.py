...
def embed_query(query,model_name):
    ...
    if query_embedding is None:
        log.warning(f'Empty Query. No query embedding generated. Ending retrieval')
        return None
...
def format_retrieval_results(raw_results):
    ...
    for ids, docs, metadatas, distances in zip(raw_results['ids'], raw_results['documents'], raw_results['metadatas'], raw_results['distances']):
        for rank, (chunk_id,doc,meta,dist) in enumerate(zip(ids,docs,metadatas,distances),start=1):
            similarity=1/(1+dist)
