...
def export_docling_to_markdown(conversion_result):
    if not conversion_result:
        log.warning("Conversion result document does not exist or is empty.")
        return None
    try:
        markdown_text = conversion_result.export_to_markdown()
        if not markdown_text:
            log.warning("Document not converted to markdown.")
            return ""
        log.info("Document successfuly converted to markdown.")
        return markdown_text
    except Exception as e:
...
def assess_extraction_quality(extracted_text, file_path):
    ...
    if char_count < 200:
        log.warning(
            f"Extracted text quality failed for {file_path}. Returned {char_count} characters. "
        )
        return False
    ...
    if alphabetic_ratio < 0.45:
        log.warning(
            f"Extracted text quality failed for {file_path}. Low alphabetic ratio {alphabetic_ratio:.2f} "
        )
        return False
    ...
    if whitespace_ratio > 0.55:
        log.warning(
            f"Extracted text quality failed for {file_path}. High whitespace ratio {whitespace_ratio:.2f} "
        )
    ...
    if weird_symbol_ratio > 0.15:
        log.warning(
            f"Extracted text quality failed for {file_path}. High weird symbol ratio {weird_symbol_ratio:.2f} "
        )
        return False
    ...
    if avg_line_len < 15:
        ...
        # return False
    log.info(
        f"Extraction passed for {file_path}:"
