...
def remove_image_artifacts(text:str)->str:
    ...
    for line in text.splitlines():
        ...
        is_image_artifact = any(re.match(pattern, stripped_line, flags=re.IGNORECASE)for pattern in image_patterns)
...
def clean_short_noise_lines(text:str)->str:
    ...
    for line in text.splitlines():
        ...
        if '<!--' in stripped_line:
            continue
        if (' ' not in stripped_line and len(stripped_line)>15 and _weird_capitalization(stripped_line)):
            continue
        cleaned_lines.append(line)
    return '\n'.join(cleaned_lines)
