...
def save_manifest(manifest: dict, manifest_path: str | Path) -> None:
    file_path=Path(manifest_path)
    if not manifest:
        log.warning(f'Manifest is empty or None. Nothing saved.')
        return
...
def update_manifest_record(manifest: dict, file_path: str | Path, document_metadata: dict,
                        file_hash: str, chunks_created: int, status: str) -> dict:
    """
    ...
    log.info(f'Manifest updated for {normalized_path} | status={status} | chunks_created={chunks_created}')
